...
with col2:
    st.header("Report")
    if disease == "NORMAL":
        response = chat_session.send_message("What are the preventive measures for pneumonia")
        st.write(response.text)
    if disease == "PNEUMONIA":
        response = chat_session.send_message("What are the medications for pneumonia")
        st.write(response.text)
